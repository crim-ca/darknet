...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char **filenames, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
  list *options = read_data_cfg(datacfg);
  char *name_list = option_find_str(options, "names", "data/names.list");
  char **names = get_labels(name_list);
  ...
  int i=0;
  while(1){
    //if a list of filenames is given
    if(filenames){
      //if the list of filenames is empty or ended we stop the execution
      if(filenames[i]==NULL){
        break;
      }
      //else the image path is the filename given in the filenames list
      else{
        strncpy(input, filenames[i],256);
        i++;
      }
    }
    //else we ask for a filename
    else {
...
void run_detector(int argc, char **argv)
{
    ...
    if (argv[6]!=NULL){
      ...
      for (i=6;i<argc;i++){
        if ((i==argc) || (argv[i]==NULL)){
          ...
          filenames[i-6]=calloc(strlen(argv[i])+1,sizeof(char));
          strncpy(filenames[i-6],argv[i],strlen(argv[i])+1);
        }
      }
    }
    else {
    ...
    if(0==strcmp(argv[2], "test")) test_detector(datacfg, cfg, weights, filenames, thresh, hier_thresh, outfile, fullscreen);
        ...
        char *name_list = option_find_str(options, "names", "data/names.list");
        char **names = get_labels(name_list);
        demo(cfg, weights, thresh, cam_index, filenames[0], names, classes, frame_skip, prefix, avg, hier_thresh, width, height, fps, fullscreen);
    }
}
